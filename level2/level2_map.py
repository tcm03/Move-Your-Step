...
def level2_play(check):
    ...
    def openFile():
        filepath = ""
        while filepath == "":
            filepath = filedialog.askopenfilename(initialdir=".\level2",title="Choose the file",filetypes= (("text files","*.txt"),("all files","*.*")))
        return filepath
    ...
    if not os.path.isdir("level2\heatmap"):
        os.mkdir("level2\heatmap")
    class Cell:
