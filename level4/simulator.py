...
def breadth_first_search(initial_map, num_keys, agent_number, positions, targets, keysets):
    ...
    if goal == (-1, -1, -1, -1):
        ...
        for next_x, next_y in find_next_moves_2D(school_map[current_f], initial_pos[1], initial_pos[2]):
            ...
            if school_map[current_f][next_x][next_y] == "0" and (current_f, next_x, next_y) not in positions.values() and (current_f, next_x, next_y) not in targets.values():
            ...
            elif school_map[current_f][next_x][next_y][0] == "D" and school_map[current_f][next_x][next_y][1] != "O" and ((1 << (int(school_map[current_f][next_x][next_y][1:])-1)) & initial_keyset) != 0:
                available_moves.append((next_x, next_y))
